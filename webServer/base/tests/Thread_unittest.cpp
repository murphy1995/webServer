...
class Foo
{
 public:
  ...
  {
    printf("tid=%d, Foo::x_=%f, text=%s\n", webServer::CurrentThread::tid(), x_, text.c_str());
  }
...
int main()
{
  ...
  webServer::Thread t2(std::bind(threadFunc2, 42),
  ...
  Foo foo(87.53);
  webServer::Thread t3(std::bind(&Foo::memberFunc, &foo),
                   "thread for member function without argument");
  t3.start();
  t3.join();

  webServer::Thread t4(std::bind(&Foo::memberFunc2, std::ref(foo), std::string("Shuo Chen")));
  t4.start();
  t4.join();

  {
    webServer::Thread t5(threadFunc3);
    t5.start();
    // t5 may destruct eariler than thread creation.
  }
  mysleep(2);
  {
    webServer::Thread t6(threadFunc3);
    t6.start();
    mysleep(2);
